...
void mtsOptoforce3D::Configure(const std::string & filename)
{
    ...
    try {
		...
		const Json::Value jsonDimension = jsonConfig["dimension"];
        if (jsonDimension.isNull()) {
            CMN_LOG_CLASS_INIT_ERROR << "Configure: cannot find \"dimension\" data in " << filename << std::endl;
            return;
        } else {
			dimension = static_cast<unsigned int>(jsonDimension.asUInt());
            CMN_LOG_CLASS_INIT_VERBOSE << "Configure: dimension (3D or 6D) = "
...
void mtsOptoforce3D::Run3D(void)
{
    struct optopacket {
...
void mtsOptoforce3D::Run6D(void)
{
    struct optopacket {
        unsigned char header[4];
        unsigned short count;
    ...
    if (connected) {
        ...
        if (valid) {
            ForceTorque.SetForce(vctDouble6((double)static_cast<short>(_byteswap_ushort(buffer.packet.fx)), 
                ...
                (double)static_cast<short>(_byteswap_ushort(buffer.packet.fz)),
                (double)static_cast<short>(_byteswap_ushort(buffer.packet.tx)),
                (double)static_cast<short>(_byteswap_ushort(buffer.packet.ty)),
                (double)static_cast<short>(_byteswap_ushort(buffer.packet.tz))));
        }
    }
    else {
        ForceTorque.SetValid(false);
